...
class TodayWidget : GlanceAppWidget() {
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        ...
        val logs = c.repository.logsSinceNow(now.minus(AgendaWindows.overdueLookback))
        val agenda = buildAgenda(protocol.phases, protocol.items, logs, now, c.zone())
        val time = DateTimeFormatter.ofPattern("HH:mm")
        val rows = (agenda.overdue + agenda.due + agenda.upcoming).mapNotNull { e ->
            val occ = e.occurrence ?: return@mapNotNull null
            val compound = protocol.compounds[occ.item.compoundId] ?: return@mapNotNull null
            WidgetRow(
                key = occ.key, name = compound.name,
                detail = "${occ.at.atZone(c.zone()).format(time)} · ${describeDose(occ.item.dose, compound.baseUnit, occ.item.formulation)}",
                color = compound.colorArgb, overdue = e.status == AgendaStatus.OVERDUE,
            )
        }
        val done = agenda.done.size
    ...
    private fun RowItem(row: WidgetRow) {
        Row(modifier = GlanceModifier.fillMaxWidth().padding(vertical = 4.dp), verticalAlignment = Alignment.CenterVertically) {
            ...
            Column(modifier = GlanceModifier.defaultWeight().clickable(actionStartActivity<MainActivity>())) {
                ...
                Text(
                    (if (row.overdue) "Overdue · " else "") + row.detail, maxLines = 1,
